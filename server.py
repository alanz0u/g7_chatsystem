...
parser.add_argument('private_key', type=str, help='The server\'s RSA private key file path')
...
async def group_message(username, group, message):
    ...
    for member_socket, member_aes_key in groups[group]:
...
async def send_file(sender_username, recipient_username, file_name, file_path):
    recipient_socket = clients[recipient_username]['socket']
    recipient_aes_key = clients[recipient_username]['aes']
    await encrypt_and_send(recipient_username, f"/file {sender_username} {file_name}", recipient_aes_key)
...
def get_client_aes_key(private_key_file:str, client_hello_msg:bytes) -> bytes:
    # decrypt aes key
    private_key = rsa_tool.load_private_key(private_key_file)
    # try:
...
async def echo(websocket, path):
    ...
    aes_key = get_client_aes_key(private_key, received_aes_key)
    if aes_key is None:
    ...
    try:
        async for message in websocket:
            ...
            if message == "/list":
                ...
                if len(parts) < 3:
                    await encrypt_and_send(websocket, f"ERROR: Invalid file transfer format. Use /file <username> <file_name>", aes_key)
                else:
                    recipient_username = parts[1]
                    file_name = parts[2]
                    if recipient_username not in clients:
                        await encrypt_and_send(websocket, f"ERROR: User '{recipient_username}' does not exist.", aes_key)
                    else:
                        file_path = await receive_file(websocket, aes_key, recipient_username, file_name)
                        await send_file(username, recipient_username, file_name, file_path)
                        await encrypt_and_send(websocket, f"File '{file_name}' sent to {recipient_username}.", aes_key)
